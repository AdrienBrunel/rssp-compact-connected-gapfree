...
function ReserveSiteSelection_SpatialConstraints(instance::Instance, gridgraph::GridGraph, params::Parameters, center::Int =0, ub::Float64 = Inf)
    ...
    function my_callback_function(cb_data, cb_where::Cint)
        ...
        for j in Noeuds
            x_val[j] = round(Int, callback_value(cb_data, x[j]))
            r_val[j] = round(Int, callback_value(cb_data, r[j]))
        end
        ...
        elseif params.is_verbose
            println("cb: solution is connected")
        end
        ...
        if isempty(Commodities_to_add) && params.is_non_reserve
            ...
            elseif params.is_verbose
                println("cb: feasible solution found, radius = $radius")
            end
        end
        if params.is_verbose && isempty(Commodities_to_add) && !params.is_non_reserve
            println("cb: feasible solution found, radius = $radius")
        end
    end
